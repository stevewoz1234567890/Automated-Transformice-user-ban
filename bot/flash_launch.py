...
After the first MAIN TCP connection (see ``FLASH_AUTO_LOGIN_UI`` in config), optional automation
can click **Dismiss all** (tunable fractions), then focus the login fields and type credentials
via ``SendInput`` (Unicode). Tune with ``FLASH_LOGIN_*`` env vars or matching keys in ``config.py``.
