...
**Automatic login (recommended):** run **`python -m bot --headless`** or set **`BOT_HEADLESS_AUTO_LOGIN=true`** in **`.env`**. The bot starts a built-in **caseus** TCP client per slot (see `bot/headless_client.py`, `HandshakePacket` → `SystemInformationPacket` → proxy-injected `LoginPacket`). **`TFM_SECRETS_*`** can be filled automatically: if those keys are incomplete, the bot runs **`tfm-secrets`** on stdout (install the CLI into your venv / PATH), then **writes the dump into `.env`** by default (`BOT_HEADLESS_SECRETS_AUTO_DUMPER`, `BOT_HEADLESS_SECRETS_PERSIST_DUMP_TO_DOTENV`). You can still set **`BOT_HEADLESS_SECRETS_INLINE_JSON`** or a custom **`BOT_HEADLESS_SECRETS_DUMPER`** argv. **`BOT_UPSTREAM_SERVER_ADDRESS`** and **`BOT_UPSTREAM_SERVER_PORTS`** can override host/ports from secrets (comma-separated ports, e.g. `11801,12801,13801,14801`).
